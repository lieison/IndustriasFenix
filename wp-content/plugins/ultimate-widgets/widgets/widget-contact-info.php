...
class uw_contact_info extends WP_Widget {
	function uw_contact_info() {
		...
		$widget_ops = array(
			'classname'		=> 'uw_widget_wrap uw_contact_info_widget',
			'description'	=> __( 'Adds support for Contact Info.', 'kho' )
		);
		// register the widget
		$this->WP_Widget('uw_contact_info', __( 'UW - Contact Info', 'kho' ), $widget_ops);

		if ( is_active_widget(false, false, $this->id_base) ) {
			if ( '1' !== uw_option( 'minify_css', '1' ) ) {
				add_action( 'wp_enqueue_scripts', array(&$this,'uw_contact_info_script'), 15);
			}
		}
	
	}

	function uw_contact_info_script() {
		wp_enqueue_style( 'uw-contact-info', uw_plugin_url( 'assets/css/styles/widgets/contact-info.css' ) );
	}
	...
	function widget($args, $instance) {
		...
		echo $before_widget;
			...
			<?php } ?>
			<ul class="contact-info-container uw-ul <?php echo esc_attr( $info_style ); ?>">
				<?php if($text) { ?>
				...
				<?php if($address) { ?>
					<li class="address">
						...
						<div class="uw-info-wrap">
							<span class="uw-contact-title"><?php _e('Address:', 'kho'); ?></span>
							<span class="uw-contact-text"><?php echo esc_attr( $address ); ?></span>
						</div>
					</li>
				<?php } ?>
				...
				<?php if($phone) { ?>
					<li class="phone">
						...
						<div class="uw-info-wrap">
							<span class="uw-contact-title"><?php _e('Phone:', 'kho'); ?></span>
							<span class="uw-contact-text"><?php echo esc_attr( $phone ); ?></span>
						</div>
					</li>
				<?php } ?>
				...
				<?php if($mobile) { ?>
					<li class="mobile">
						...
						<div class="uw-info-wrap">
							<span class="uw-contact-title"><?php _e('Mobile:', 'kho'); ?></span>
							<span class="uw-contact-text"><?php echo esc_attr( $mobile ); ?></span>
						</div>
					</li>
				<?php } ?>
				...
				<?php if($fax) { ?>
					<li class="fax">
						...
						<div class="uw-info-wrap">
							<span class="uw-contact-title"><?php _e('Fax:', 'kho'); ?></span>
							<span class="uw-contact-text"><?php echo esc_attr( $fax ); ?></span>
						</div>
					</li>
				<?php } ?>
				...
				<?php if($email) { ?>
					<li class="email">
						...
						<div class="uw-info-wrap">
							<span class="uw-contact-title"><?php _e('Email:', 'kho'); ?></span>
				...
				<?php if($web) { ?>
					<li class="web">
						...
						<div class="uw-info-wrap">
							<span class="uw-contact-title"><?php _e('Website:', 'kho'); ?></span>
							<span class="uw-contact-text">
				...
				<?php if($skype) { ?>
					<li class="skype">
						<a href="skype:<?php echo esc_attr( $skype ); ?>?call" target="_self" class="uw-skype-button">
	...
	function form($instance) {
		...
		<p>
			...
			<select class='uw-widget-select widefat' name="<?php echo $this->get_field_name('info_style'); ?>" id="<?php echo $this->get_field_id('info_style'); ?>">
