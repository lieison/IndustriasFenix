...
class uw_mailchimp extends WP_Widget {
	...
	public function __construct() {
        ...
        parent::__construct(
            'uw_mailchimp',
            $name = __( 'UW - MailChimp', 'kho' ),
            array(
                'classname'		=> 'uw_widget_wrap uw_newsletter_widget',
        ...
        if ( is_active_widget(false, false, $this->id_base) ) {
			if ( '1' !== uw_option( 'minify_css', '1' ) ) {
				add_action( 'wp_enqueue_scripts', array(&$this,'uw_mailchimp_script'), 15);
			}
		}

    }

	public function uw_mailchimp_script() {
		wp_enqueue_style( 'uw-mailchimp', uw_plugin_url( 'assets/css/styles/widgets/mailchimp.css' ) );
	}
	...
	public function widget($args, $instance) {
		...
		echo $before_widget;
			...
			<?php } ?>
			<p class="uw-mail-text" style="<?php echo esc_attr( $center_text ); ?>"><?php echo esc_attr( $subscribe_text ); ?></p>
	...
	public function form( $instance ) {
		...
		<p>
			...
			<select class='uw-widget-select widefat' name="<?php echo $this->get_field_name('center_text'); ?>" id="<?php echo $this->get_field_id('center_text'); ?>">
