...
if( !class_exists( 'YITH_WC_Audio_Video' ) ){
    ...
    class YITH_WC_Audio_Video
    {
        ...
        public function add_video_field()
        {
            ...
            echo yith_wcfav_get_template('admin/add_simple_url_video.php', $args, true);
        }
            ...
            if ( $post_type == 'product' ) {
                ...
                if( in_array( $video_host['host'], $this->hosts['youtube'] ) ){
                    ...
                    $data_setup =   array(
                        ...
                        'src'         =>   stripslashes( $video_url ),
                    );

                    $args['data_setup'] =   $data_setup;
                    $args['atts']   =   $args;


                    return yith_wcfav_get_template( 'template_video_player.php', $args, true );

                }elseif( in_array( $video_host['host'], $this->hosts['vimeo'] ) ){

                    $data_setup =   array(
                        'techOrder'   => array( 'vimeo' ),
                        'src'         =>    stripslashes( $video_url )
        ...
        public function include_style_and_script_free(){
            ...
            wp_enqueue_script( 'videojs', YWCFAV_ASSETS_URL.'js/external_libraries/video.js', array('jquery'), false, true );
            wp_enqueue_script( 'media_youtube', YWCFAV_ASSETS_URL.'js/external_libraries/media.youtube.js', array('videojs'), false, true  );
            wp_enqueue_script( 'media_vimeo', YWCFAV_ASSETS_URL.'js/external_libraries/media.vimeo.js', array('videojs'), false, true  );

            wp_enqueue_style( 'videojs_style', YWCFAV_ASSETS_URL. 'css/videojs/video-js.min.css' );
            wp_enqueue_style( 'ywcfav_style', YWCFAV_ASSETS_URL. 'css/ywcfav_frontend.css' );
        }
    }
